...
class Utils(object):
    ...
    def convert_csv_to_multivariate_timeseries(directory):
        ...
        for file in files_list:
            ...
            multivariate_timeseries = MultivariateTimeSeries.generate_from_file(directory, file)
    ...
    def json_dict(representation, name, dictos, key='list'):
        ...
        for k, i in dictos.items():
            if key == 'list':
                representation = Utils.json_list(representation, k, i)
                representation += ','
            else:
                representation += '"' + k + '": '
    ...
    #     files_list = [f for f in os.listdir(directory + Utils.DIRNAME) if f.lower().endswith(Utils.EXTENSION)]
    #     list_shapelets = []
    #     for file in files_list:
    #         path = directory + Utils.DIRNAME + file
    ...
    def dataset_entropy(unid):
        ...
        for timeseries in unid:
            classes_dict[timeseries.class_timeseries.lower()].append(timeseries)
        entropy = 0.0
        for key in classes_dict.keys():
            p = float(len(classes_dict[key])) / len(unid)
            entropy += Utils.entropy_helper(p)
        return entropy
    ...
    def entropy_after_split(unid1, unid2):
        ...
        entropy1 = f1 * Utils.dataset_entropy(unid1)
        entropy2 = f2 * Utils.dataset_entropy(unid2)
    ...
    @staticmethod
    def min_length_dataset(list_multivariate_timeseries):
        min_l = sys.maxsize
    ...
    def generate_new_color(existing_colors, pastel_factor=0.5):
        ...
        for i in range(0, 100):
            color = Utils.get_random_color(pastel_factor=pastel_factor)
            if not existing_colors:
                return color
            best_distance = min([Utils.color_distance(color, c) for c in existing_colors])
    ...
    def generate_new_colors(k=10):
        colors = []
        for i in range(k):
            colors.append(Utils.generate_new_color(colors, pastel_factor=0.9))
        return colors